...
general_task = \
"""You are generating an evalauation dataset for a question answering system.
Given a piece of text extracted from a source document, you must come up with a related question that the system's users may ask.
Then provide the correct answer to this question based on the given text sample.
"""

custom_task = \
"""You are generating an evalauation dataset for a question answering system based on a car user manual.
Given a piece of text extracted from the manual, you must come up with a related question that a car user might be asking while troubleshooting, performing maintanance, or discovering features.
Then provide the correct answer to this question based on the given text sample.
"""
...
----------------
{text}"""
CHAT_GENERATION_PROMPT = ChatPromptTemplate.from_messages(
    [
...
----------------
{text}"""
GENERATION_PROMPT = PromptTemplate.from_template(templ)

GENERATION_PROMPT_SELECTOR = ConditionalPromptSelector(
    default_prompt=GENERATION_PROMPT, conditionals=[(is_chat_model, CHAT_GENERATION_PROMPT)]
)
